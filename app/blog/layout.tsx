      ...
      <div className='mt-12 flex items-center flex-col justify-center'>
        ...
        <div className='flex items-center justify-center gap-4 mt-4'>
          <TwitterShareButton
            ...
            title={currentPost?.title || 'Interesting blog post about JavaScript'}
          >
            <TwitterIcon size={32} round />
          </TwitterShareButton>
          <FacebookShareButton
            url={typeof window !== 'undefined' ? `${window.location.origin}${pathname}` : ''}
            quote={`${currentPost?.title || 'Check out this blog post'} - ${currentPost?.description || 'A deep dive into JavaScript concepts'}`}
            hashtag={
              currentPost?.tags && currentPost.tags.length > 0 ? currentPost.tags.map(tag => `#${tag.replace(/\s+/g, '')}`).join(' ') : '#blog'
            }
          >
          ...
          <RedditShareButton
            url={typeof window !== 'undefined' ? `${window.location.origin}${pathname}` : ''}
            title={currentPost?.title || 'Interesting JavaScript tutorial'}
          >
